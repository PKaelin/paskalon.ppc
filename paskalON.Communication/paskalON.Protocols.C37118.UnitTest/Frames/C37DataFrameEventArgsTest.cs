...
namespace paskalON.Protocols.C37118.UnitTest.Frames
{
    ...
    public class C37DataFrameEventArgsTest
    {
        ...
        {
            byte[] data = C37TestDataGenerator.CreateMockDataFrame(1, new List<(float Mag, float Ang)>(), new List<float>(), 0);
        ...
        {
            byte[] data = C37TestDataGenerator.CreateMockDataFrame(1, new List<(float Mag, float Ang)> { (1, 2) }, new List<float>(), 10);
            C37DataFrameEventArgs dataFrame = new C37DataFrameEventArgs(data);
        ...
        {
            byte[] data = C37TestDataGenerator.CreateMockDataFrame(1, new List<(float Mag, float Ang)>(), new List<float> { 1 }, 10);
            C37DataFrameEventArgs dataFrame = new C37DataFrameEventArgs(data);
