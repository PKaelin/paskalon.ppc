    ...
    public class DerUnitPowerConstraint : PowerConstraintBase, IDerUnitConstraint
    {
        ...
        public override void ApplyConstraints(ref ActivePower activePower, ref ReactivePower reactivePower)
        {
            if (_config.IsEnabled)
            {
                ...
                {
                    _logger.LogWarning("{Name} units active power {ActivePower} exceeds maximum limit {MaxLimit}. Clamping to maximum.", Name, activePower.KiloWatts, _config.MaximumActivePowerKiloWatt);
                ...
                {
                    _logger.LogWarning("{Name} units active power {ActivePower} below minimum limit {MinLimit}. Clamping to minimum.", Name, activePower.KiloWatts, _config.MinimumActivePowerKiloWatt);
                ...
                {
                    _logger.LogWarning("{Name} units reactive power {ReactivePower} exceeds maximum limit {MaxLimit}. Clamping to maximum.", Name, reactivePower.KiloVoltAmperesReactive, _config.MaximumReactivePowerKiloVars);
                ...
                {
                    _logger.LogWarning("{Name} units reactive power {ReactivePower} below minimum limit {MinLimit}. Clamping to minimum.", Name, reactivePower.KiloVoltAmperesReactive, _config.MinimumReactivePowerKiloVars);
