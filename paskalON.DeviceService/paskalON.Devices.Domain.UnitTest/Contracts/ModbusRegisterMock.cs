    ...
    internal class ModbusRegisterMock
    {
        ...
        internal void Setup<TDevice, TProperty>()
        {
            ...
            Mock.Setup(x => x.Register<TDevice, TProperty>(
                    ...
                    It.IsAny<WordOrder>(),
                    It.IsAny<int>()))
                .Callback((TDevice instance, string name, Action<TDevice, TProperty> setter, int register, double scale, WordOrder wordOrder, int offset) =>
                {
                    ArgumentNullException.ThrowIfNull(instance);
                    Registrations[name] = new ModbusRegisterEntry<TDevice, TProperty>(instance, name, setter, register, scale, wordOrder, offset);
                });
        }
    }
}
